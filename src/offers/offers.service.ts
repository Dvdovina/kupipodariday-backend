    ...
    async create(createOfferDto: CreateOfferDto, userId: number) {
        ...
        try {
            ...
            if (wish.owner.id === userId) {
                throw new BadRequestException('Скидываться можно только на подарки другим пользователям');
            }

            const raised: number = amount + wish.raised;
            if (raised > wish.price) {
                throw new BadRequestException({ message: 'Сумма превышает стоимость подарка', raised: raised, price: wish.price });
            }
            wish.raised = raised;
